...
class PrawdziwyZegar3_9:
    def __init__(self, root):
        ...
        self.lbl_nowy_czas = tk.Label(self.ramka_cyfrowa, text="", font=("Courier New", 20, "bold"))
    ...
    def aktualizuj_zegar(self):
        ...
        # W godzinach 3:00 - 9:00 wskazówka godzinowa jest na dole
        if 3 <= h < 9:
            sfera = "[-]"
            bg_color = "#1e293b" # Ciemny granat (Noc/Dół)
            fg_color = "#f8fafc"
            line_color = "#475569"
        else:
            sfera = "[+]"
            bg_color = "#f8fafc" # Jasny (Dzień/Góra)
            fg_color = "#0f172a"
            line_color = "#cbd5e1"
        ...
        # Oś horyzontalna (3-9) - nasz horyzont
        ...
        # Obliczanie minuty ortogonalnej (unikalnej)
        kat_m_mod = kat_m % 180
        kat_h_mod = kat_h % 180
        ...
        unikalna_minuta = abs(90 - rozny_kat)
        
        # Wykrywanie "Momentu Ortos" (Zielony błysk zbieżności)
        if unikalna_minuta < 1.5:
        ...
        self.lbl_nowy_czas.config(
            text=f"{sfera} {h%12}h | Δ {unikalna_minuta:.2f}°"
        )
